...
#define TWEAKS_VERSION "3.19.369"
...
static const char* const kTwkShoveFix  = "TwkCatchShoveFixes";
...
static void pageValue(const char* key, int iv, float fv, void*) {
    ...
    else if (!strcmp(key, kTwkShoveFix))  CatchTweaks_SetShoveFixes(iv != 0);
...
static int pageGet(const char* key, int* oi, float* of, void*) {
    ...
    else if (!strcmp(key, kTwkShoveFix))  { *oi = CatchTweaks_ShoveFixes() ? 1 : 0; return 1; }
...
static const OmpPageItem2 kTwkCatchItems[] = {
    ...
    // Bug fixes have no row -- always on, ini kill-switch only, like CatchUnstick and CatchPlantFix:
    // CatchStopsFlip (catch ends the flip), CatchAnyRevolution (foot always attaches),
    // CatchFootLevelsBoard (over-rotation leveling). Twelve rows; the window is thirteen.
};
