// SessionOpenMP: a copy of Dear ImGui's DX12 renderer backend, v1.90.9-docking
// (backends/imgui_impl_dx12.cpp at 3369cbd2776d7567ac198b1a3017a4fa2d547cc3, MIT licence, see
// https://github.com/ocornut/imgui/blob/master/LICENSE.txt), compiled INSTEAD of the fetched one.
//
// ONE CHANGE, and it only touches the pop-out windows (the F1 menu on a second monitor): the game's
// own swapchain is drawn by exactly the upstream code. Upstream presents each extra window and then
// SPINS until the GPU has finished drawing it. Here that runs inside the game's Present, on the game's
// render thread, and the GPU is still busy with the game's frame -- so every frame the menu was
// popped out, the game would wait for its own GPU work to drain. Instead each frame slot remembers
// the fence value of its last use, and the wait happens only when that SAME slot comes round again,
// numFramesInFlight frames later, by which time it is done. Search "OMP:" for the edits.
...
#include "imgui.h"
#include "imgui_internal.h"     // OMP: ImGui_ImplDX12_OmpWaitViewports walks every viewport
...
struct ImGui_ImplDX12_FrameContext
{
    ...
    UINT64                          FenceValue;     // OMP: fence value of this slot's last submit (0 = never)
};
...
static void ImGui_ImplDX12_CreateWindow(ImGuiViewport* viewport)
{
    ...
    if (dxgi_factory && vd->CommandQueue)       // OMP: guarded
    {
        res = dxgi_factory->CreateSwapChainForHwnd(vd->CommandQueue, hwnd, &sd1, nullptr, nullptr, &swap_chain);
        IM_ASSERT(res == S_OK);
        // OMP: DXGI would otherwise watch this window's messages and take Alt+Enter as a fullscreen
        // switch -- a mode change started from the game's render thread.
        if (swap_chain)
...
static void ImGui_ImplDX12_RenderWindow(ImGuiViewport* viewport, void*)
{
    ImGui_ImplDX12_Data* bd = ImGui_ImplDX12_GetBackendData();
    ImGui_ImplDX12_ViewportData* vd = (ImGui_ImplDX12_ViewportData*)viewport->RendererUserData;
    if (!vd || !vd->SwapChain || !vd->CommandList || !vd->Fence || !vd->FenceEvent)   // OMP
        return;

    // OMP: the allocator slot is the one RenderDrawData below picks for its vertex/index buffers
    // (it increments FrameIndex first), so one fence value covers both. Upstream indexed the allocator
    // by the index BEFORE the increment, which only worked because SwapBuffers waited for everything.
    ImGui_ImplDX12_FrameContext* frame_context = &vd->FrameCtx[(vd->FrameIndex + 1) % bd->numFramesInFlight];
    if (frame_context->FenceValue != 0 && vd->Fence->GetCompletedValue() < frame_context->FenceValue)
    {
        ::WaitForSingleObject(vd->FenceEvent, 0); // Reset any forgotten waits
        vd->Fence->SetEventOnCompletion(frame_context->FenceValue, vd->FenceEvent);
        ::WaitForSingleObject(vd->FenceEvent, INFINITE);
    }
    ...
    frame_context->FenceValue = vd->FenceSignaledValue;   // OMP
}

static void ImGui_ImplDX12_SwapBuffers(ImGuiViewport* viewport, void*)
{
    ImGui_ImplDX12_ViewportData* vd = (ImGui_ImplDX12_ViewportData*)viewport->RendererUserData;
    if (!vd || !vd->SwapChain)                  // OMP
        return;

    vd->SwapChain->Present(0, 0);
    // OMP: no wait here -- RenderWindow waits per slot, a whole ring of frames later.
}

// OMP: with SwapBuffers no longer waiting, a pop-out frame can still be on the GPU when the device
// objects -- the font texture above all -- are thrown away. Called before InvalidateDeviceObjects.
void ImGui_ImplDX12_OmpWaitViewports()
{
    ImGuiContext* ctx = ImGui::GetCurrentContext();
    if (!ctx)
        return;
    for (int i = 1; i < ctx->Viewports.Size; i++)
        ImGui_WaitForPendingOperations((ImGui_ImplDX12_ViewportData*)ctx->Viewports[i]->RendererUserData);
}
